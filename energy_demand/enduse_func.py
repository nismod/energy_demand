...
class Enduse(object):
    ...
    def __init__(
        ...
        if np.sum(fuel) == 0:
            ...
            self.fuel_peak_h = 0
            self.enduse_techs = []
            self.fuel_supply_generated = 0 # MAYBE ARRAY
        else:
            ...
            if self.enduse_techs == []:
                ...
                """
                self.fuel_supply_generated = 0 #TODO
                ...
                # -------------------------------------------
                self.fuel_y, fuel_tech_y, self.fuel_supply_generated = service_to_fuel(
                    enduse,
                    s_tech_y_by,
                    s_tech_y_cy,
                    tech_stock,
                ...
                if self.flat_profile_crit:
                    ...
                    # NEW
                    '''self.fuel_supply_generated_yh = calc_fuel_tech_yh(
                        enduse,
                        sector,
                        ['tech_CHP'], #TODO ADD TECHS
                        self.fuel_supply_generated,
                        load_profiles,
                        fueltypes_nr,
                        fueltypes,
                        model_yeardays_nrs,
...
def assign_lp_no_techs(enduse, sector, load_profiles, fuel_y):
    ...
    fuel_peak_dh = fuel_y[:, np.newaxis] * f_peak_yd * shape_peak_dh
...
def service_to_fuel(
    ...
    fuel_per_tech : dict
        Fuel per technology
    fuel_supply_generated : array
        Fuel which is generated by technology (e.g. electrificty for CHP)
    ...
    fuel_supply_generated = np.zeros((fueltypes_nr), dtype=float)
    ...
    if mode_constrained:
        for tech, service in service_tech.items():
            ...
            '''if tech_type == 'tech_CHP':

                # ---------------------------------------
                # Generated additional fuel by technology
                # ---------------------------------------
                eff_fuel_generated_by = tech_stock.get_tech_attr(
                    enduse, tech, 'eff_elec_by')

                eff_fuel_generated_cy = tech_stock.get_tech_attr(
                    enduse, tech, 'eff_elec_cy')

                fueltype_generation_int = tech_stock.get_tech_attr(
                    enduse, tech, 'fueltype_generation_int')

                # Total fuel generated cy
                fuel_generated_cy = service / eff_fuel_generated_cy
                fuel_generated_by = service_tech_by[tech] / eff_fuel_generated_by

                # Additional fuel since by 
                additional_fuel_generated_cy = fuel_generated_cy - fuel_generated_by

                # May be minus because of lower service
                if additional_fuel_generated_cy < 0:
                    additional_fuel_generated_cy = 0
                    logging.info("ABZUG fuel")

                # Calculate fuel which is generated by technoogy
                fuel_supply_generated[fueltype_generation_int] += additional_fuel_generated_cy

                # --------
                # TODO: NOT CORRECT DO NOT DO THIS
                # --------
                if additional_fuel_generated_cy > 0:
                    logging.info("{}".format(enduse))
                    logging.info("Elec in by: {}  elec in cy: {} total diff: {}  int: {}".format(
                        fuel_generated_by, fuel_generated_cy, additional_fuel_generated_cy, fueltype_generation_int))
                    logging.info("before: " + str(fuel_y))

                # Substract generated additional fuel for this enduse
                fuel_y[fueltype_generation_int] -= additional_fuel_generated_cy

                if additional_fuel_generated_cy > 0:
                    logging.info("after : " + str(fuel_y))

                if fuel_y[fueltype_generation_int] < 0:
                    raise Exception("The additional fuel generated by CHP is larger than actual use for this enduse")'''
     ...
     #logging.info("TEST sdf" + str(fuel_supply_generated))
    return fuel_y, fuel_tech_y, fuel_supply_generated
