...
def scenario_initalisation(path_data_ed, data=False):
    ...
    if run_locally is True: #EVerything removed or data needs to be read in from somewhere locally
        ...
        data['print_criteria'] = True #Print criteria
        data['paths'] = data_loader.load_paths(path_main)
        data['local_paths'] = data_loader.load_local_paths(path_data_ed)
        data['lookups'] = data_loader.load_basic_lookups()
        data['enduses'], data['sectors'], data['fuels'] = data_loader.load_fuels(
            data['paths'], data['lookups'])
    ...
    if run_locally is True: #TODO REMOVE
        s_fuel_to_service.run(data['assumptions'], data['lookups'], data['fuels'], data['enduses'], data['local_paths'])
    else:
        fts_cont = {}
    ...
    if run_locally is True:
        s_disaggregation.run(data) #TODO REMOVE
    else:
        sd_cont = {}
